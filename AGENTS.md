...
- **Airtime orders** (live since 0.4.4) — order reference prefix `AIR-`, payment collected via Lipila MoMo prompt, fulfilled through a **pluggable provider layer** (`src/airtime.ts`: `manual` default, `mtn_momo` for Zambia, `africastalking` for other markets — AT does NOT offer the airtime product for Zambia). `airtime_orders` carries `lipila_reference` / `attempts` / `error` / `credits_used_cents` (migration_v26). Enable via `app_settings.airtime_enabled` (off by default). Admin airtime screen shows the active provider badge + provider-aware K1 test; manual-mode orders are confirmed delivered via the dashboard.
...
- **Smart notifications overhaul (0.7.0)** — mirror of the ChurchOnApp pattern: **11 per-category channels** (`giving_updates`, `events`, `support`, `payments`, `admin`, `promotions`, `sponsor_desk`, `broadcast`, `referrals`, `host`, `links`), every one `Importance.max` + sound + vibration so alerts ALWAYS pop (never silent). Backend sends the matching `channelId` per type (`channelForType` in `firebase.ts`); the app mirrors it in `_channelForType`. Content-keyed dedup (30s window) + burst coalescing (queued pushes collapse to one summary per channel) stop duplicate/stacking alerts. Notifications show the app icon (`@mipmap/ic_launcher` large icon) + brand-orange `#E65100`, a big-text style, an "Open" action button, and every type has a deep-link route (`_routeFromData`). Foreground FCM messages also fire an **in-app slide-down banner** (`NotificationOverlay` in `main.dart`'s builder) so nothing is missed while the app is open.
- **Notifications show time** — `safeDateTime()` in `date_utils.dart` renders UTC `created_at` as local "14 Aug 2026 · 14:30" on the notifications list + detail dialog.
- **Sponsor Desk** — host screen at `/sponsor-desk` (curated grant/empowerment opportunities; tap to apply via external link), admin screen at `/admin/sponsor-desk` (create/edit/archive/publish batches to active hosts, `settings` scope, tile in Admin → Tools & settings). Backend pushes `type: "sponsor_desk"` on publish → `sponsor_desk` channel + tap routes to the screen. Admin stats now include `sponsorDeskTotal`/`sponsorDeskPublished`/`sponsorDeskActive`.
